<template>
  <div class="deptMumbers">
    <div class="deptName">
      {{sectionName?sectionName:'部门名'}}<span><a @click="addSubDept('new')" v-if="this.sectionType!=13">添加子部门</a><a
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      await axiosget(`/hospital/department/admin/getSectionListByUserHosId?userHosId=${this.userHosId}`).then(res => {
        if (res.data) {
    ...
    methods: {
      ...
      comfirDept() {
        ...
        axiospost(`hospital/member/admin/insertDoctor`, self.multipleSelection[0]).then(
          res => {
            if (res.data) {
              axiosget(
                `/hospital/member/admin/getUserDoctorList?hosId=${this.hosId}&sectionId=${this.sectionId}&searchStr=${this.infoSearch}`
