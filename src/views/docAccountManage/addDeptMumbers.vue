<template>
  <div v-loading="saveLoading" class="deptMumbers">
    <div class="deptName">
      ...
      <div class="admBtns">
        <el-button 
          v-if="queryFormType!='edit'"
          type="success" 
          @click="saveDoctor('doctorForm', 'continue')">保存并继续添加</el-button>
        <el-button 
          type="primary" 
          plain 
          @click="saveDoctor('doctorForm', '')">保存</el-button>
        <el-button @click="bankToList">取消</el-button>
      </div>
    </div>
    <div class="admInfoWrite">
      <!-- <div v-loading="photoLoading" class="docLogo">
        <el-upload
            action=""
        ...
        <el-col :span="20">
          <el-form :model="doctorForm" :rules="rules" ref="doctorForm" label-width="150px" class="widthInitial">
            ...
            <el-form-item label="科室" :rules="[ { required: true, message: '请选择科室'}]">
              <el-button 
                v-show="pageDeptList.length==0"
                type="text" 
                class="text-underline"
                @click="selectDept">选择科室</el-button>
              <div v-show="pageDeptList.length>0">
                <el-tag
                  class="dept-tag"
                  v-for="(item, index) of pageDeptList"
                  :key="index"
                  closable
                  @close="pageDeleteTag(item.id)">
                  {{ item.name }}
                </el-tag>
                <el-button 
                  type="text" 
                  class="text-underline"
                  @click="selectDept">修改</el-button>
              </div>
            </el-form-item>
          </el-form>
        </el-col>
        ...
        <el-main class="dept-choose-dialog-layout">
          <el-scrollbar class="scrollbar">
            <div class="dept-wrap">
              ...
              <el-tag
                class="dept-tag"
                v-for="(item, index) of Object.keys(deptList)"
                :key="index"
                :disable-transitions="true"
                closable
                @close="deleteTag(item, 2)">
                {{ deptList[item] }}
              </el-tag>
            </div>
          </el-scrollbar>
...
<script>
  import { axiosget, axiospost, aixospatch } from "../../service/config.js"
  import { uploadImgApi } from '../../service/application.js'
  import { insertDoctorApi } from '../../service/hospital.js'
  import {
    mapGetters,
    mapState,
    mapActions
  ...
  export default {
    ...
    methods: {
      ...
      getMemberInfo() {
        let self = this;
        axiosget(
          `/hospital/member/admin/getDoctorInfo?hosId=${self.userHosId}&userId=${self.queryUserId}&phone=${self.queryPhone}`
        ).then(res => {
          if (res.data) {
      ...
      saveDoctor(formName, type) {
        ...
        self.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            //console.log('提交前内容：', self.doctorForm);
            ...
            //console.log('111111111', submitForm.userId);
            insertDoctorApi(
              submitForm
            ).then((res) => {
