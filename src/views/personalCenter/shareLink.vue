...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      this.addr.value = `${exportLocalUrl}/personalCenter/confirmInviteMsg/${invitationInfo.invitationCode}?orgname=${encodeURI(escape(JSON.parse(sessionStorage.getItem('orgnameS'))))}`;
    },
    methods: {
      copyUrl(e) {
